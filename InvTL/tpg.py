...
class LambdaRegExpEvaluator:
    ...
    def match (self,input,pos):
        #assert False
        for i in range(len(input)-pos):
            for j in range(len(input)-pos):
                start=i+pos
                end=len(input)-j
                if self.F(input[start:end]):
...
class ContextSensitiveLexer(LexerOptions):
    ...
    def def_token(self, name, expr, value=_id):
        ...
        it is returned whatever the text of the token.
        """
        #print "DEF_TOKEN: ",name, expr, value
        #if expr=='\$[a-zA-Z]\w*' : print 0/0
        if not callable(value):
            value = lambda _, value=value: value
        if name not in self.tokens and name not in self.separators:
            if len(expr)>0 and expr=="$call$":
                import parser
                self.tokens[name] = LambdaRegExpEvaluator(value), _id
            else:
    ...
    def eat(self, name):
        ...
        #print "Matching: ", self.tokens[name], self.pos
        if tok is None:
            raise WrongToken
        else:
            if self.cur_token is None:

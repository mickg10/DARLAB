    ...
    def compute_hash (self,*params):
        ...
        for i in params:
            ...
            if isinstance(i,str) or isinstance(i,int) or isinstance(i,float):
                hash.append(getHash("%s_%s"%(str(i.__class__.__name__),str(i))))
            elif isinstance(i,tuple) or isinstance(i,list):
                hash.append(getHash("%s_%s"%(str(i.__class__.__name__),repr(i))))
            else:
                hash.append(getHash("%s_%s"%(str(i.__class__.__name__),pickle.dumps(i))))
                #print "Z"
            #print hash
        #print z
        if len(params)==1:
            prefix=str(params[0].__class__.__name__)
            z=hash[0]
        else:
            z=getHash("_".join(hash))
        ret="id_%s_%s"%(prefix,z)
        return ret
    def load(self,*params,**options):
        broken=False
        hash=self.compute_hash(params)
        #print hash
        try:
            data=self.__getitem__(hash)
            try:
                #print "DSA"
                t=pickle.loads(data)
                #print "ASD"
            ...
            raise inst
        return None
    def save(self,data,*params,**options):
        h=self.compute_hash(params)
        #print h
        try:
            self.__setitem__(h,pickle.dumps(data))
